...
package io.github.earthchen.playground;

import io.github.earthchen.playground.functions.BackpressureMap;
import io.github.earthchen.playground.functions.ClickEventStatisticsCollector;
import io.github.earthchen.playground.functions.CountingAggregator;
import io.github.earthchen.playground.records.ClickEvent;
import io.github.earthchen.playground.records.ClickEventDeserializationSchema;
import io.github.earthchen.playground.records.ClickEventStatistics;
import io.github.earthchen.playground.records.ClickEventStatisticsSerializationSchema;
...
/**
 * A simple streaming job reading {@link ClickEvent}s from Kafka, counting events per 15 seconds and writing the
 * resulting {@link ClickEventStatistics} back to Kafka.
 *
 ...
 * * "--checkpointing": enables checkpointing * "--event-time": set the StreamTimeCharacteristic to EventTime *
 * "--backpressure": insert an operator that causes periodic backpressure * "--input-topic": the name of the Kafka Topic
 * to consume {@link ClickEvent}s from * "--output-topic": the name of the Kafka Topic to produce {@link
 * ClickEventStatistics} to * "--bootstrap.servers": comma-separated list of Kafka brokers
 */
public class ClickEventCount {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String brokers = params.get("bootstrap.servers", "192.168.200.159:9094");
        ...
        DataStream<ClickEvent> clicks =
                env.addSource(new FlinkKafkaConsumer<>(inputTopic, new ClickEventDeserializationSchema(), kafkaProps))
                        ...
                        .assignTimestampsAndWatermarks(
                                new BoundedOutOfOrdernessTimestampExtractor<ClickEvent>(Time.of(200,
